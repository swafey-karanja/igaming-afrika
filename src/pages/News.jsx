...
const News = () => {
  ...
  const skeletons = Array(displayLimit).fill(0).map((_, i) => (
    <div key={i} className="bg-white rounded-2xl shadow-md overflow-hidden flex animate-pulse">
  ...
  return (
    <>
        <Header />
        <div className="bg-gray-100 py-26 px-4 sm:px-6 lg:px-8 h-auto">
            <div className="max-w-7xl mx-auto">
                <div className="pt-12 bg-gray-100">
                    <h2 className="text-4xl font-semibold text-gray-800 mb-2">Read Latest Articles & News</h2>
                    <hr />
                    <p className="text-gray-600"> Discover the latest free game giveaways and news from the gaming world.</p>
                </div>

                <div className={`mt-8 grid gap-6 grid-cols-1 sm:grid-cols-2 transition-opacity duration-500 ${
                        loading ? 'opacity-0' : 'opacity-100'
                    }`}>
                    {/* Conditional rendering: Show skeletons if loading, else show news */}
                    {loading ? skeletons : news.slice(0, displayLimit).map((article) => ( (
                        <div
                        key={article.id}
