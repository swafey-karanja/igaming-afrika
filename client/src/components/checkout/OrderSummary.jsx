...
import { Calendar, MapPin, Clock, Check, Shield } from "lucide-react";
// eslint-disable-next-line no-unused-vars
import { motion } from "framer-motion";

const OrderSummary = ({ selectedTicket }) => (
  <motion.div
    className="bg-white rounded-lg shadow-sm p-6 relative top-0 mb-5"
  ...
  >
    ...
    <div className="mb-6">
      <div className="flex justify-between items-start mb-3">
        <div>
          <h3 className="font-medium text-gray-900">{selectedTicket.label}</h3>
      ...
      <div className="mb-4">
        ...
        <ul className="text-sm text-gray-600 space-y-1">
          {selectedTicket.features.slice(0, 4).map((feature, idx) => (
            <li key={idx} className="flex items-center">
              <Check className="w-3 h-3 text-green-600 mr-2 flex-shrink-0" />
              {feature}
            </li>
          ))}
          {selectedTicket.features.length > 4 && (
            <li className="text-green-600 text-xs">
              +{selectedTicket.features.length - 4} more features
            </li>
          )}
        </ul>
      </div>
